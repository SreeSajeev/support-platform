...
const ITHelpdeskView: React.FC = () => {
  ...
  const [psNumberFilter, setPsNumberFilter] = useState('');
    ...
    const fetchProblems = async () => {
      ...
      try {
        const res = await fetch(
          'https://reimagined-space-eureka-q7qrj6xwwx6qcxpjr-5000.app.github.dev/api/helpdesk-view/problems'
        );
        ...
        setProblemsError(err.message || 'Unknown error');
      } finally {
  ...
  const handleSearch = () => {
    ...
    if (psNumberFilter.trim()) {
      filtered = filtered.filter(p =>
        p.psNumber.toLowerCase().includes(psNumberFilter.toLowerCase())
      );
    }
      ...
      <div className="max-w-[1366px] w-full px-4 py-8">
          ...
          <motion.div className="flex flex-col md:flex-row gap-4 mb-6" variants={itemVariants}>
            <div className="flex-1">
              ...
              <input
                ...
                value={psNumberFilter}
                onChange={e => setPsNumberFilter(e.target.value)}
              />
            </div>
            ...
            <div className="flex-1">
              ...
              >
                ...
                <option value="Open">Infrastructure</option>
                <option value="In Progress">Application</option>
              </select>
            </div>
        ...
        >
          {problemsLoading ? (
            ...
            <p className="p-6 text-center text-lt-grey">No problems found.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {filteredProblems.map(problem => (
                  <TableRow key={problem.id} className="hover:bg-lt-lightGrey">
                    ...
                    <TableCell>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleGoToTicket(problem.id)}
                      >
                        View
                      </Button>
                    </TableCell>
